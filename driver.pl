...
while ($line = <INFILE>) {
    ...
    if ($inpuzzles) {
       ...
       # absolute kludge to make performance points 5 instead of 2;
       # a good fix requires modifying elsewhere...
       if ( $puzzle_p_points{$name} == 2 ) {
			 $puzzle_p_points{$name} = 5;
		 }
       $total_p_points += $puzzle_p_points{$name};
       # we're awarding up to 5 performance points per puzzle
       $total_p_rating += 5;
    }
}
close(INFILE);
...
print "\nPart 4: Score Report\n\n";
print "\n";
printf("%s\n", "Correctness Results   Perf Results   Logic");
printf("%s\n", "Points     Errors     Points   Ops    Desc*  Puzzle");
printf("------------------------------------------------------------\n");
foreach $name (sort {$puzzle_number{$a} <=> $puzzle_number{$b}} 
          keys %puzzle_number) {
      printf("%6d%11d%11d%6d%8d   %s\n", 
...
printf("------------------------------------------------------------\n");
printf("* to be entered manually by a TA\n");

$tpoints = $total_c_points + $total_p_points;
$trating = $total_c_rating + $total_p_rating;
print "\nAuto Score = $tpoints/$trating [$total_c_points/$total_c_rating Corr + $total_p_points/$total_p_rating Perf] ($tops total operators)\n";

$logicMax = $total_c_points / 2;
printf("Logic description deduction:    0 / %2d\n", $logicMax);
print("Total score =   / 100\n");

#
# Write score information in a format suitable to be "harvested" by my
# Harvester.jar tool: 
#
#          NN >> description of test: score
#
printf("Harvestable version of the function scores:\n\n");
my $testNum;
$testNum = 1;

printf(">>Scores from testing<<\n");
foreach $name (sort {$puzzle_number{$a} <=> $puzzle_number{$b}} 
          keys %puzzle_number) {
   printf("%d >> %s correctness: %d / %d\n", 
      $testNum,
      $name,
      $puzzle_c_points{$name}, 
      $puzzle_c_rating{$name});
   $testNum++;
   printf("%d >> %s performance: %d / %d\n", 
      $testNum,
      $name,
      $puzzle_p_points{$name},
      5);
